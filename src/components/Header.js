...
export default function Header({ title = 'داشبورد', showBack = false }) {
  ...
  if (!showBack) {
    return (
      <div className="sticky top-0 z-50 bg-gray-50 pt-6 pb-3">
        <motion.header 
          initial={{ y: -50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 24 }}
          className="mx-4 p-2 bg-white/80 backdrop-blur-xl border border-white rounded-[28px] flex items-center justify-between shadow-sm"
        >
  ...
  return (
    <div className="sticky top-0 z-50 bg-gray-50 pt-4 pb-3">
      <motion.header 
        initial={{ y: -20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        className="mx-4 p-2 bg-white/80 backdrop-blur-xl border border-white rounded-[24px] flex items-center justify-between shadow-sm"
      >
        <button 
          onClick={() => router.back()} 
